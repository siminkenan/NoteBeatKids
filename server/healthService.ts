...
export interface HealthReport {
  ...
  postgresql: { status: "ok" | "error" | "slow"; pingMs: number | null; poolTotal: number; poolIdle: number; poolWaiting: number };
  redis: { status: "ok" | "disabled" | "error"; memoryUsedMB: number | null; hitRate: number | null; keyCount: number | null; connected: boolean };
  ...
  monthlyReset: { lastRunAt: string | null; lastResult: "success" | "failed" | "skipped" | null; nextRunAt: string | null };
...
async function checkPostgres(): Promise<HealthReport["postgresql"]> {
  ...
  try {
    ...
    return { status: pingMs > 2000 ? "slow" : "ok", pingMs, poolTotal: stats.total, poolIdle: stats.idle, poolWaiting: stats.waiting };
  } catch {
    return { status: "error", pingMs: null, poolTotal: stats.total, poolIdle: stats.idle, poolWaiting: stats.waiting };
  }
}

async function checkRedis(): Promise<HealthReport["redis"]> {
  if (!redis) return { status: "disabled", memoryUsedMB: null, hitRate: null, keyCount: null, connected: false };
  try {
    ...
    try { const dbInfo = await redis.info("keyspace"); const match = dbInfo.match(/keys=(\d+)/); if (match) keyCount = parseInt(match[1]); } catch {}
    return { status: redis.status === "ready" ? "ok" : "error", memoryUsedMB: Math.round(memBytes / 1024 / 1024), hitRate, keyCount, connected: redis.status === "ready" };
  } catch {
    return { status: "error", memoryUsedMB: null, hitRate: null, keyCount: null, connected: false };
  }
}
...
export async function getHealthReport(): Promise<HealthReport> {
  ...
  const overall: HealthReport["overall"] = pg.status === "error" ? "critical" : rd.status === "error" || pg.status === "slow" ? "degraded" : "healthy";
  return {
    timestamp: new Date().toISOString(), overall,
    postgresql: pg, redis: rd, socketio: socketStats,
    scoreBuffer: { pendingEntries: bufferSize(), dirtyInstitutions: dirtyInstitutionCount(), lastFlushAt: flushStats.lastFlushAt?.toISOString() ?? null, lastFlushDurationMs: flushStats.lastFlushDurationMs, lastFlushSuccess: flushStats.lastFlushSuccess, lastFlushError: flushStats.lastFlushError },
    leaderboard: { lastBroadcastAt: leaderboardStats.lastBroadcastAt?.toISOString() ?? null, lastBroadcastDurationMs: leaderboardStats.lastBroadcastDurationMs },
    monthlyReset: { lastRunAt: monthlyResetStats.lastRunAt?.toISOString() ?? null, lastResult: monthlyResetStats.lastResult, nextRunAt: monthlyResetStats.nextRunAt?.toISOString() ?? null },
    api: apiMetrics,
